...
class ScanManga : ParsedHttpSource() {
    ...
    // --- POPULAIRES ---
    override fun popularMangaRequest(page: Int): Request = GET("$baseUrl/top-mangas.html", headers)
    override fun popularMangaSelector(): String = "div.content_manga div.element, div.listing_manga div.manga_item"
    override fun popularMangaFromElement(element: Element): SManga = SManga.create().apply {
        val titleElement = element.select("div.title a, h3 a, a.manga_title").first()!!
    ...
    override fun searchMangaRequest(page: Int, query: String, filters: FilterList): Request =
        GET("$baseUrl/recherche?q=${java.net.URLEncoder.encode(query, "UTF-8")}", headers)
    ...
    override fun chapterFromElement(element: Element): SChapter = SChapter.create().apply {
        ...
        name = link.text().trim()
        date_upload = parseDate(element.select("span.date, span.time").text().trim())
    }

    private fun parseDate(dateStr: String): Long {
        return runCatching {
            SimpleDateFormat("dd/MM/yyyy", Locale.FRANCE).parse(dateStr)?.time ?: 0L
        }.getOrDefault(0L)
    }

    // --- DÉCODAGE DES PAGES / IMAGES ---
    override fun pageListParse(document: Document): List<Page> {
        val pages = mutableListOf<Page>()
        val html = document.html()

        // 1. Recherche d'URLs directement intégrées dans les scripts JS
        val scriptPattern = Pattern.compile("var\\s+(?:images|pages|lstImages)\\s*=\\s*\\[(.*?)\\];", Pattern.DOTALL)
        val matcher = scriptPattern.matcher(html)

        if (matcher.find()) {
            val rawArray = matcher.group(1) ?: ""
            val urlPattern = Pattern.compile("['\"](https?://[^'\"]+|\"/[^'\"]+)['\"]")
            val urlMatcher = urlPattern.matcher(rawArray)
            var index = 0
        ...
        // 2. Repli statique si aucune variable JS n'est trouvée
        if (pages.isEmpty()) {
        ...
        if (pages.isEmpty()) {
            throw Exception("Impossible d'extraire les pages : protection JS active ou sélecteurs obsolètes.")
        }
